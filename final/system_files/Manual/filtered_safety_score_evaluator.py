...
def generate_graph(perfect_trajectory_array, ideal_trajectory_length, driven_path_array, driven_path_length, game_time,
    ...
    plt.savefig(f'step7/scenario_{scenario_num}_results.png', dpi=300)
...
def generate_score_distribution_graph(rated_scenario_data):
    ...
    # Save the figure as an image
    plt.savefig(f'step7/calculated_safety_score_dist.png', dpi=300)
...
rated_objects = []
count = 0
for root, dirs, files in os.walk('step5'):
    for file in files:
        if 'manual_scenario' in file:
        
            # Extract the number from the file name
            scenario_num = int(file.split('_')[-1].split('.')[0])

            # Open the corresponding route file
            with open(f'step4/filtered_route_{scenario_num}.json', 'r') as f:
                trajectory = json.load(f)

            with open(f'step5/manual_scenario_{scenario_num}.json') as f:
...
with open('step6/rated_scenarios.json', 'w') as f:
    json.dump(rated_objects, f,indent=4)

with open("step6/rated_scenarios.json", "r") as file:
